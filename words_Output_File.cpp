...
ofstream out("output_words.txt",ios::out);

//报错(row,col,文字报错类型)
void error(int i,int j,string errortype="未知"){
    out<<endl<<"Fault: in row "<<i<<" column "<<j<<" 报错原因: "<<errortype<<"\n"; 
}

//打印二元式(开始,结束,类型)
void print(int start,int end,int type){
    out<<'('<<type<<','<<str.substr(start,end-start+1)<<','<<row<<','<<start+1<<')'<<endl;
}
...
int main()
{
    ...
    ifstream file("demo/demoQ.txt",ios::in);
    if(!file){
        ...
        while(getline(file,str)){
            ...
            while(k<len){
                ...
                //字符串开始 左"
                    ...
                    for(;k<len;++k){
                        if(str[k]=='"'){
                            ...
                            else    out<<"字符串右半边处理出错，请检查！";
                    
                            stringflag=0;
                            out<<'('<<3<<','<<str.substr(start,k+1-start)<<','<<a.row<<','<<a.column<<')'<<endl;
                            k++;
                            break;
                        }
                    }
                    ...
                    while(k<len){
                        while(isalpha(str[k]) || str[k]=='_' || isdigit(str[k])){
                            if(sumdig>9){//超数字个数
                                ...
                                out<<'('<<1<<','<<str.substr(start,k-start)<<','<<row<<','<<start+1<<')'<<endl;
                                flag=0;
                            }
                            if(k-start>32){
                                error(row,col,"字符总数超过32个");
                                out<<'('<<1<<','<<str.substr(start,32)<<','<<row<<','<<start+1<<')'<<endl;
                                flag=0;
                            }
                            if(!flag)break;
                ...
                // id 1 或者 保留字 2
                    ...
                    if(x=="if" || x=="or")  {out<<'('<<2<<','<<x<<','<<row<<','<<col<<')'<<endl;k+=2;continue;}
                    else{
                        x=str.substr(k,3);//int and
                        if(x=="int" ||x=="and"){out<<'('<<2<<','<<x<<','<<row<<','<<col<<')'<<endl;k+=3;continue;}
                        else{
                            x=str.substr(k,4);//else    void
                            if(x=="else" || x=="void"){out<<'('<<2<<','<<x<<','<<row<<','<<col<<')'<<endl;k+=4;continue;}
                            else{
                                x=str.substr(k,5);//float   while   break
                                if(x=="float" ||x=="while" || x=="break" ){out<<'('<<2<<','<<x<<','<<row<<','<<col<<')'<<endl;k+=5;continue;}
                                else{
                                    x=str.substr(k,6);//string  return
                                    if(x=="string" ||x=="return"){out<<'('<<2<<','<<x<<','<<row<<','<<col<<')'<<endl;k+=6;continue;}
                                    ...
                                    //id处理
                                        ...
                                        else {out<<'('<<1<<','<<str[start]<<','<<row<<','<<start+1<<')'<<endl;}
                                    }
                                }
                            }
                        }
